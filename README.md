...
- 权限审批体验：当 AI 再次发送新的权限请求之前，如果上一条请求已在桌面端被撤销或超时，MobileVC 会即时识别并给出「请求已失效」提示，不再触发无意义的热替换操作。
- 会话时间线瘦身：终端日志流会自动剥离 `Wall time` 报头、空的 `Output:` 包裹和重复性噪音，只保留真正需要你关注的 stderr/stdout 内容。
- 后台提醒更可靠：当 app 处于 `inactive`（前后台切换过渡期）接收到权限或回复通知时，会先排队等待，直到完全进入后台后再推送，避免错过关键信号。
