...
// entry count.
func ExtractJSONLEvents(entries []store.SnapshotLogEntry, startIndex int) ([]JSONLEvent, int) {
...
func MergeJSONLToSession(cwd, claudeUUID string, existingEntries []store.SnapshotLogEntry) (newEntries []store.SnapshotLogEntry, newCount int, _ error) {
	...
	var merged []store.SnapshotLogEntry
	...
	allEntries := make([]store.SnapshotLogEntry, 0, len(existingEntries)+len(merged))
	allEntries = append(allEntries, existingEntries...)
	allEntries = append(allEntries, merged...)
	return merged, len(allEntries), nil
}

func entryDedupKey(e store.SnapshotLogEntry) string {
	text := strings.TrimSpace(e.Message)
	if text == "" {
		text = strings.TrimSpace(e.Text)
	}
